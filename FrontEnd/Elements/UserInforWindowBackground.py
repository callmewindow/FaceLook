...
class UserInforWindowBackground(Element):
    ...
    def init(self):
        ...
        self.remark1 = self.createChild(text_variable, (175, 60),self.tempUser.get("remark"), 'simhei', 20, (100,100,100))
        self.editButton = self.createChild(TripleStateButton, (270, 135), './resources/UserInforWinUI/edit.png', (30, 30))
        self.chatButton = self.createChild(TripleStateButton, (330, 135), './resources/UserInforWinUI/chat.png', (30, 30))

        # 添加具体信息
        topY = 210
        self.remark2 = self.createChild(InforBar, (100, topY-30), "备注", self.tempUser.get("remark"), 16)
    ...
    def update(self):
        ...
        if self.tempUser.get("uid") == self.uid:
            self.chatButton.disable()
            self.remark1.disable()
            self.remark2.disable()
